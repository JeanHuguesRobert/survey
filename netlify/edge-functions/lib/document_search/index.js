...
async function reformulateQuery(userQuestion, history) {
  ...
  `;
    ...
    const contextMessages = history.slice(-4).map(m => `${m.role}: ${m.content}`).join("\n");
 ...
 */
export async function getDocContext(userQuestion, history = [], scope = DocumentSearchConfig.FILE_SEARCH_DEFAULT_STORES) {
    try {
        ...
        console.log(`[DocumentSearch] Query: "${userQuestion}" -> Rewritten: "${rewrittenQuery}"`, filters);
