...
- Write intent (job & step) to persistent storage before performing external actions or publishing
  events (write-ahead).
- Use `source_event_id` on `cop_job` to deduplicate job creation for the same incoming event.
- Use unique indexes (job_id+name) for `cop_step` and (source_job_id,source_step_id,type) for
  `cop_artifact` to make upserts idempotent.
- When processing a step, set `cop_step.status` to `running` and publish a `job_step_started` event
  before heavy work.
...
- Agents should be stateless and reconstruct their state from DB: always query
  `cop_job`/`cop_step`/`cop_artifact` to know progress.
