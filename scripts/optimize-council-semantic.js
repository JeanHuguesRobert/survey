...
function jsonToCompactMarkdown(councilsData) {
  ...
  for (const data of councilsData) {
    ...
    const presentsDisplay = presents.length > 6 
    ...
    if (data.deliberations?.length) {
      ...
      const importantes = data.deliberations.filter(d => 
        d.importance === 'haute' || 
        d.vote.contre > 0 || 
        d.vote.abstention > 2 ||
    ...
    if (data.debats?.length) {
      ...
      for (const deb of data.debats) {
        md += `**${deb.sujet}**\n`;
        deb.intervenants.forEach(int => 
          md += `- ${int.nom}: ${int.position}\n`
        );
        md += `→ *${deb.decision}*\n\n`;
      }
    }
...
async function main() {
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    try {
      ...
      allData.push(data);
      console.log(`[${i + 1}/${sections.length}] ✅ ${data.date} | ${data.deliberations?.length || 0} délib.`);
    } catch (error) {
