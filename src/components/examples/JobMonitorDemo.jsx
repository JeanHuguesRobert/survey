...
export default function JobMonitorDemo() {
  // Use job actions from context or direct import
  const [createJob, setCreateJob] = useState(null);
  const [updateJobProgress, setUpdateJobProgress] = useState(null);
  React.useEffect(() => {
    import("../../contexts/SupabaseContext").then((mod) => {
      setCreateJob(() => mod.createJob);
      setUpdateJobProgress(() => mod.updateJobProgress);
    });
  }, []);
      ...
      <div className="   shadow-md p-6">
        ...
        {(job || hookJob) && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-100">Current Job</h3>
            <JobProgress
              job={job || hookJob}
        ...
        <div className="mt-8 bg-blue-50 border border-blue-200   p-4">
          ...
          <div className="text-sm text-blue-700 space-y-2">
            <p>
              <strong>1. Create a job:</strong> Use <code>createJob(type, payload)</code> to create
              a new job
            </p>
            <p>
              <strong>2. Monitor progress:</strong> Use <code>useJobMonitor(jobId)</code> hook to
              subscribe to updates
            </p>
            <p>
              <strong>3. Update progress:</strong> Call{" "}
              <code>updateJobProgress(jobId, progress, message, status)</code>
            </p>
            <p>
              <strong>4. Handle completion:</strong> The hook provides callbacks for progress,
              completion, and errors
            </p>
            <p>
              <strong>5. Realtime updates:</strong> All job changes are broadcast via Supabase
              realtime channels
            </p>
          </div>
        </div>
