...
export default function VotingDashboard() {
  ...
  const loadStats = async () => {
    ...
    const [
      ...
      supabase.from('propositions').select('*, proposition_tags(tag:tags(*))').eq('status', 'active'),
    ...
    if (propositionsRes.data && votesRes.data && delegationsRes.data && usersRes.data && tagsRes.data) {
      const propositions = propositionsRes.data;
      const votes = votesRes.data;
      const delegations = delegationsRes.data;

      const propositionStats = propositions.map(prop => {
      ...
      const tagStats = Object.entries(tagVotes).map(([name, data]) => ({
        name,
      ...
      const delegationStats = Object.entries(delegationsByTag).map(([name, value]) => ({
        name,
        value
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard title="Propositions actives" value={stats.totalPropositions} color="bg-blue-900" />
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-xl font-bold text-gray-900 mb-4">Top 10 propositions par nombre de votes</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-xl font-bold text-gray-900 mb-4">Distribution globale des votes</h3>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-xl font-bold text-gray-900 mb-4">Tags les plus actifs</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      {stats.delegationStats.length > 0 && (
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-xl font-bold text-gray-900 mb-4">Délégations par tag</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 p-4 rounded-md">
            <p className="text-sm text-gray-600">Moyenne de votes par proposition</p>
            <p className="text-3xl font-bold text-blue-900">
              {stats.totalPropositions > 0 ? (stats.totalVotes / stats.totalPropositions).toFixed(1) : 0}
            </p>
          </div>
          <div className="bg-green-50 p-4 rounded-md">
            <p className="text-sm text-gray-600">Moyenne de votes par utilisateur</p>
            <p className="text-3xl font-bold text-green-700">
              {stats.totalUsers > 0 ? (stats.totalVotes / stats.totalUsers).toFixed(1) : 0}
            </p>
          </div>
          <div className="bg-yellow-50 p-4 rounded-md">
            <p className="text-sm text-gray-600">Taux de délégation</p>
            <p className="text-3xl font-bold text-yellow-700">
              {stats.totalUsers > 0 ? ((stats.totalDelegations / stats.totalUsers) * 100).toFixed(0) : 0}%
            </p>
          </div>
        </div>
      </div>
