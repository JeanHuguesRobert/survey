...
export default function GlobalDashboard({ currentUser }) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Check for errors
      const errors = [propositionsRes, votesRes, delegationsRes, wikiPagesRes, postsRes, commentsRes, chatInteractionsRes]
      ...
      const filledFields = profileFields.filter(field => currentUser[field] && currentUser[field].trim() !== '');
  ...
  const fetchRecentActivity = async (userId) => {
    try {
      ...
      const [propositions, wikiPages, posts, comments, chatInteractions] = await Promise.all([
        supabase.from('propositions').select('id, title, created_at').eq('author_id', userId).order('created_at', { ascending: false }).limit(5),
        supabase.from('wiki_pages').select('id, title, created_at').eq('author_id', userId).order('created_at', { ascending: false }).limit(5),
        ...
        supabase.from('chat_interactions').select('id, question, created_at').eq('user_id', userId).order('created_at', { ascending: false }).limit(5)
      ]);
  ...
  const generateContributionData = () => {
    const data = [
      { name: 'Kudocracy', value: stats.kudocracy.propositions + stats.kudocracy.votes + stats.kudocracy.delegations },
  ...
  const calculateActivityScore = () => {
    const total = stats.kudocracy.propositions + stats.kudocracy.votes + stats.kudocracy.delegations +
                 stats.wiki.pages + stats.social.posts + stats.social.comments + stats.chat.interactions;
    return total;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Global Dashboard</h1>
          <p className="text-gray-600">Overview of your activities across all platform modules</p>
        </div>

        {/* Overall Statistics */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Kudocracy</h3>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Wiki</h3>
            <div className="space-y-2">
              <div className="flex justify-between">
                <span className="text-gray-600">Pages Created:</span>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-sm">
                  {stats.profile.joinDate ? new Date(stats.profile.joinDate).toLocaleDateString() : 'N/A'}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Contribution Metrics</h3>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Contribution Distribution</h3>
              ...
              recentActivity.map((activity) => (
                <div key={activity.id} className="flex items-start space-x-4 p-4 border border-gray-200 rounded-lg">
                  <div className={`w-3 h-3 rounded-full mt-2 ${
                    activity.type === 'proposition' ? 'bg-blue-500' :
                    activity.type === 'wiki' ? 'bg-green-500' :
                    activity.type === 'post' ? 'bg-purple-500' :
                    activity.type === 'comment' ? 'bg-yellow-500' :
                    'bg-gray-500'
                  }`}></div>
                  <div className="flex-1">
                    <Link to={activity.link} className="font-medium text-gray-900 hover:text-orange-600">
                      {activity.title}
                    </Link>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            >
              ...
              <span className="font-medium text-gray-900">Kudocracy</span>
              <span className="text-sm text-gray-500">Create propositions</span>
            </Link>
            ...
            >
              ...
              <span className="text-sm text-gray-500">Edit pages</span>
            </Link>
            ...
            >
              ...
              <span className="text-sm text-gray-500">Share posts</span>
            </Link>
