    ...
    <div className={`bg-${statusColor}-50 border border-${statusColor}-200 rounded-lg p-4`}>
      ...
      {showDetails && (
        <div className="mt-3 text-xs text-gray-600 space-y-1">
          ...
          {job.created_at && (
            <div><strong>Créé:</strong> {new Date(job.created_at).toLocaleString()}</div>
          )}
          {job.started_at && (
            <div><strong>Démarré:</strong> {new Date(job.started_at).toLocaleString()}</div>
          )}
          {job.completed_at && (
            <div><strong>Terminé:</strong> {new Date(job.completed_at).toLocaleString()}</div>
          )}
          {job.started_at && (
            <div><strong>Durée:</strong> {formatDuration(job.started_at, job.completed_at)}</div>
          )}
          {job.error_details && (
