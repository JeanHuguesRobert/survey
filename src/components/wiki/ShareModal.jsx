...
const ShareModal = ({ isOpen, onClose, pageTitle, pageUrl, pageContent }) => {
  const [shareText, setShareText] = useState(`Découvrez la page "${pageTitle}" sur le Wiki de la consultation citoyenne: ${pageUrl}`);
  ...
  useEffect(() => {
    if (isOpen) {
      setShareText(`Découvrez la page "${pageTitle}" sur le Wiki de la consultation citoyenne: ${pageUrl}`);
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full m-4">
        ...
        <div className="mb-4">
          <label htmlFor="shareText" className="block text-gray-700 text-sm font-bold mb-2">Texte à partager:</label>
          <textarea
            id="shareText"
