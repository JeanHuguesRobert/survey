// Netlify Edge Function (Deno) to claim and process a single COP job
...
export default async function handler(request) {
  ...
  try {
    ...
    const rpcRes = await fetch(`${SUPABASE_URL}/rest/v1/rpc/cop_claim_job`, {
      method: "POST",
      headers: {
    ...
    if (!rpcRes.ok)
      return new Response(JSON.stringify({ error: "No job claimed" }), { status: 204 });
    const jobs = await rpcRes.json();
    if (!jobs || jobs.length === 0)
      return new Response(JSON.stringify({ ok: "no jobs" }), { status: 200 });
    const job = jobs[0];
    // Fetch next pending step
    const stepsRes = await fetch(
      `${SUPABASE_URL}/rest/v1/cop_step?job_id=eq.${job.id}&status=eq.pending&order=created_at.asc&limit=1`,
      {
        method: "GET",
    ...
    if (!step) {
      // Nothing to do, mark job done if required
      await fetch(`${SUPABASE_URL}/rest/v1/cop_job?id=eq.${job.id}`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${SERVICE_KEY}`,
          apikey: SERVICE_KEY,
      ...
      return new Response(JSON.stringify({ ok: "job done" }), { status: 200 });
    }
    // Mark step running and publish job_step_started write-ahead
    ...
    const existingArtRes = await fetch(
      `${SUPABASE_URL}/rest/v1/cop_artifact?source_job_id=eq.${job.id}&source_step_id=eq.${step.id}&type=eq.deep_answer`,
    ...
    // If no more pending steps, mark job done
    const pendingStepsRes = await fetch(
      `${SUPABASE_URL}/rest/v1/cop_step?job_id=eq.${job.id}&status=eq.pending`,
      { method: "GET", headers: { Authorization: `Bearer ${SERVICE_KEY}`, apikey: SERVICE_KEY } }
    );
    ...
    return new Response(
      JSON.stringify({ ok: "processed step", jobId: job.id, stepId: step.id, artifactId: art?.id }),
      { status: 200 }
    );
  } catch (e) {
    console.error("process-job edge error", e?.message || e);
    // Attempt to update job: clear lease so it can be claimed again
    try {
      if (jobs && jobs[0]) {
        await fetch(`${SUPABASE_URL}/rest/v1/cop_job?id=eq.${jobs[0].id}`, {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${SERVICE_KEY}`,
            apikey: SERVICE_KEY,
      ...
      console.warn("failed to clear job lease", e2?.message || e2);
    }
