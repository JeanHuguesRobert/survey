...
export default function Transparence() {
  ...
  const handleSave = async (event) => {
    ...
    if (data && data.length) {
      ...
      setFormData({ ...initialFormState, ...updatedEntry, population: updatedEntry.population ?? '' });
    ...
    setSuccessMessage('Merci ! Votre contribution enrichit la base nationale de la transparence municipale.');
    setFormOpen(false);
  };
        ...
        <section className="bg-white rounded-xl shadow p-6 md:p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="border border-slate-200 rounded-lg p-5 text-center">
              ...
              <p className="text-xs text-slate-500 mt-2">Complétez le formulaire pour mesurer et partager votre score.</p>
            </div>
          </div>
        </section>
          ...
          <div>
            ...
            <div className="flex flex-col md:flex-row gap-3 md:items-center">
              <div className="flex-1">
                <label className="sr-only" htmlFor="commune">Nom de votre commune</label>
                <input
                  id="commune"
                  list="commune-list"
            ...
            {loading && (
              <p className="text-sm text-slate-500 mt-3">Chargement des communes...</p>
            )}
            {error && (
          ...
          <div className="border border-dashed border-slate-200 rounded-lg p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {CRITERIA.map((criterion) => (
                <label
                  key={criterion.key}
                  className="flex items-start gap-3 rounded-lg border border-slate-200 p-4 bg-slate-50"
                >
                  <input
                    type="checkbox"
        ...
        {formOpen && (
          <section className="bg-white rounded-xl shadow p-6 md:p-8">
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
              <div>
                <h2 className="text-xl font-semibold text-slate-900">Créer ou mettre à jour une fiche</h2>
            ...
            <form className="space-y-6" onSubmit={handleSave}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-slate-700 mb-1" htmlFor="commune_name">
                    Nom de la commune
                  </label>
                  <input
                    id="commune_name"
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1" htmlFor="insee_code">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1" htmlFor="population">
                    Population (≥ 3 500)
                  </label>
                  <input
                    id="population"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1" htmlFor="submitted_by">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <p className="text-sm text-slate-500">
                  → En contribuant, vous participez à la création d\'une base de données nationale de la transparence municipale.
                </p>
                <button
                  type="submit"
                  disabled={saving}
                  className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-orange-500 text-white font-semibold hover:bg-orange-600 disabled:opacity-60"
                >
        ...
        <section className="bg-white rounded-xl shadow p-6 md:p-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-sm text-slate-600">
                {communes.length} commune{communes.length > 1 ? 's' : ''} contribuent déjà à l\'observatoire.
              </p>
            </div>
            ...
            <table className="min-w-full border border-slate-200 divide-y divide-slate-200 text-sm">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  {CRITERIA.map((criterion) => (
                    <th key={criterion.key} className="px-4 py-2 text-left font-semibold text-slate-600">
                      {criterion.label}
                    </th>
                  ))}
                </tr>
