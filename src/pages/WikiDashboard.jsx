...
export default function WikiDashboard() {
  ...
  const loadWikiStats = async () => {
    ...
    try {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const monthEdits = userRevisions?.filter(r => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Chargement de vos contributions Wiki...</p>
        </div>
      </div>
    );
  }
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-gray-600">Veuillez vous connecter pour voir vos contributions Wiki.</div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-2">Vos contributions Wiki</h1>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Éditions totales</p>
                <p className="text-2xl font-bold text-gray-900">{stats?.totalEdits || 0}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Pages créées</p>
                <p className="text-2xl font-bold text-gray-900">{stats?.totalPagesCreated || 0}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Pages éditées</p>
                <p className="text-2xl font-bold text-gray-900">{stats?.uniquePagesEdited || 0}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Pages Created */}
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Pages créées</h2>
            {createdPages.length > 0 ? (
              <div className="space-y-3">
                {createdPages.map((page) => (
                  <div key={page.id} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                    <div>
                      <Link
                        to={`/wiki/${page.slug}`}
          ...
          {/* Most Edited Pages */}
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Pages les plus éditées</h2>
            {stats?.mostEditedPages?.length > 0 ? (
              <div className="space-y-3">
                {stats.mostEditedPages.map((page, index) => (
                  <div key={index} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                    <span className="font-medium text-gray-900">{page.title}</span>
                    <span className="text-sm text-gray-600">{page.count} édition{page.count > 1 ? 's' : ''}</span>
                  </div>
                ))}
              </div>
            ) : (
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mt-8">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Éditions récentes</h2>
          {recentEdits.length > 0 ? (
            <div className="space-y-4">
              {recentEdits.map((edit) => (
                <div key={edit.id} className="flex justify-between items-start py-3 border-b border-gray-100 last:border-b-0">
                  <div className="flex-1">
                    ...
                    {edit.comment && (
                      <p className="text-sm text-gray-600 mt-1">{edit.comment}</p>
                    )}
                  </div>
