    ...
    try {
      ...
      if (groupedSubs.post?.length > 0) {
        const { data: postComments } = await supabase
          .from('comments')
          .select('*, users(id, display_name, email), posts!inner(id, metadata)')
          .in('post_id', groupedSubs.post)
          .order('created_at', { ascending: false })
          .limit(50);

        if (postComments) {
          allComments.push(...postComments.map(c => ({
            ...c,
            content_type: 'post',
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-md p-12 text-center max-w-md">
          <p className="text-gray-600 mb-4">
            Vous devez être connecté pour voir vos abonnements
          </p>
          <Link
            to="/"
            className="inline-block px-6 py-3 bg-blue-900 text-white rounded-md hover:bg-blue-800"
          >
            Retour à l'accueil
          </Link>
        </div>
                ...
                <div key={comment.id} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="text-sm text-gray-600 mb-2">
                    <span className="font-medium">
                      {comment.users?.display_name || comment.users?.email || 'Anonyme'}
                    </span>
                    {' '}a commenté :
                  </div>
